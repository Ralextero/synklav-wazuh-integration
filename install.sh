...
print(" 3) UPDATE ALERTS   : Change the minimum alert level for an existing node.")
...
if exec_profile in [1, 2]:
    ...
    if len(recovery_kit.split()) != 24:
        ...
        sys.exit(1)
    min_alert_level = input("➔ Enter the MINIMUM alert level for FCM routing (1-15): ").strip()
...
if exec_profile in [1, 2]:
    ...
    # Mantenemos el mismo cálculo criptográfico para asegurar sincronía con el Flutter app
    tag_hash = hashlib.sha256((notification_key + node_uid).encode('utf-8')).hexdigest()

target_name = f"custom-synklav-{node_uid}"

# ------------------------------------------------------------------------------
# MOTOR DE EXTRACCIÓN LÍNEA A LÍNEA
# ------------------------------------------------------------------------------
...
for line in raw_lines:
    ...
    if in_integration:
        ...
        if exec_profile == 5:
            if "custom-synklav" in line: is_synklav_integration = True
        elif exec_profile == 4:
            if target_name in line: is_synklav_integration = True
...
if exec_profile in [1, 2]:
    # SCRIP DE PYTHON CREADO EN LA MÁQUINA (Purgado de basura OneSignal)
...
NODE_UID, NOTIFICATION_KEY, TAG_HASH, TG_CHAT_ID = "{node_uid}", "{notification_key}", "{tag_hash}", "{tg_chat_id}"
